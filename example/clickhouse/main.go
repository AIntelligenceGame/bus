...
func main() {
	...
	if err != nil {
		zap.L().Error(fmt.Sprintf("获取数据库实例连接，失败：%v", err), zap.String("clickhouse", "8123"))
		panic(err)
	}
	defer db.Close()
	...
	err = sqlutils.QueryRowsMap(db, `select * from tablename where key = ?`, func(m sqlutils.RowMap) error {
		zap.L().Info("数据库返回信息：", zap.String("_time", m.GetString("_time")))
		zap.L().Info("数据库返回信息：", zap.String("instanceid", m.GetString("instanceid")))
		zap.L().Info("数据库返回信息：", zap.String("projectName", m.GetString("projectName")))
		return nil
	}, "key-value")
